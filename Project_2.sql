...
FROM sales 
GROUP BY product ORDER BY avg_price DESC;


SELECT payment_type, SUM(price) AS total_price
FROM sales
GROUP BY payment_type ORDER BY total_price DESC;



SELECT country, AVG(price) AS avg_price FROM sales
GROUP BY country HAVING avg_price>1500 ORDER BY avg_price;
...
SELECT MAX(price) AS max_price, country FROM sales
GROUP BY country ORDER BY max_price DESC;


SELECT payment_type, SUM(price) AS total_price FROM sales
GROUP BY payment_type HAVING total_price > 50000;

SELECT name, product, price, payment_type FROM sales
WHERE payment_type = 'Visa' OR payment_type = 'Mastercard';

SELECT product, COUNT(*) FROM sales 
WHERE payment_type = 'Mastercard'
GROUP BY product;

SELECT latitude, product,longitude, price
FROM sales
WHERE latitude > 37 AND longitude > 12;


SELECT SUM(price) AS total_price, 
CASE
    WHEN latitude < 0  THEN 'Southern Hemisphere'
    WHEN latitude = 0 THEN 'Equitorial'
